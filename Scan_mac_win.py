    ...
    for l in list_ip:
        ...
        for i in my_scan.list_of_hosts_found:
            try:
                ...
                tipo2 = tipo[:-9]
                if tipo2 == '20:87:56':
                    mach_siem =('Machine: Siemens' + '\tip: ' + i + '\tMAC: ' + gma(ip=i, network_request=True) + '\tName: ' +  (socket.getfqdn(i)))
                    list_machine.append(mach_siem)
                elif tipo2 == '00:a0:cd' :
                    mach_heid = ('Machine: Heidenhain' + '\tip: ' + i + '\tMAC: ' + gma(ip=i, network_request=True) + '\tName: ' +  (socket.getfqdn(i)))
